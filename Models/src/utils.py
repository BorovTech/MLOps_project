...
def concat_df(df1: pd.DataFrame, df2: pd.DataFrame,shuffle=True):
    ...
    '''
    dataframes =[df1,df2]
    assert all([len(dataframes[0].columns.intersection(df.columns)) == dataframes[0].shape[1] for df in dataframes]), "The dataframes' structure must be identical"
    if shuffle == False:
...
def select_grid(model_name: str):
    ...
    if model_name == 'XGBClassifier':
        return { 'learning_rate': [0.1, 0.01, 0.001],'max_depth': [3, 5, 7], 'subsample': [0.8, 0.9, 1.0], 'colsample_bytree': [0.8, 0.9, 1.0]}
    if model_name == "DecisionTreeClassifier":
        return { 'criterion': ['gini', 'entropy'],'max_depth': [None, 5, 10, 20],'min_samples_split': [2, 5, 10],'min_samples_leaf': [1, 2, 4],'max_features': ['sqrt', 'log2', None]}
    if model_name == "LogisticRegression":
        return { 'C': [0.001, 0.01, 0.1, 1, 10, 100], 'penalty': ['none','l2']}
    if model_name == 'RandomForestClassifier':
        return {'n_estimators': [25,50,100],'criterion': ['gini', 'entropy'],'max_depth': [None,3,5], 'min_samples_split': [2, 5, 10]}
